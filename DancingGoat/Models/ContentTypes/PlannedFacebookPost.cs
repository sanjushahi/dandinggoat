// This code was generated by a cloud-generators-net tool 
// (see https://github.com/Kentico/cloud-generators-net).
// 
// Changes to this file may cause incorrect behavior and will be lost if the code is regenerated. 
// For further modifications of the class, create a separate file with the partial class.

using System;
using System.Collections.Generic;
using KenticoCloud.Delivery;

namespace DancingGoat.Models
{
    public partial class PlannedFacebookPost
    {
        public const string Codename = "planned_facebook_post";
        public const string MessageCodename = "message";
        public const string PlannedPublishDateCodename = "planned_publish_date";

        public string Message { get; set; }
        public DateTime? PlannedPublishDate { get; set; }
        public ContentItemSystemAttributes System { get; set; }
    }
}